...
void print_Linked_List(Node *head)
{
    ...
    cout<<endl<<"Your Awesome Linked List : ";
    while(temp != NULL)
    {
        cout<<temp->val<<" ";
        temp = temp->next;
...
void insert_At_Tail(Node * &head, int val) 
{
    ...
    {
        head = newNode;
        cout<<endl<<"Inserted the first node"<<endl<<endl;
        return ;
    }
...
void insert_At_Position(Node * head,int pos,int val) // done
{
    ...
    for(int i = 1; i<= pos-1; i++)
    {
        ...
        if(temp == NULL)
        {
            cout<<endl<<"Huh! Temp is at NULL.Null doesn't have next."<<endl<<endl;
            return;
        }
    }
...
void delete_from_position(Node * head,int pos) // done
{
    ...
    if(temp->next == NULL )
    {
        cout<<endl<<"Invalid Index.temp's next is also null"<<endl<<endl;
        return;
    }
